...
void ofApp::draw()
{
    m_tileSaver.begin(); //-------------------------------------------------------------- TILE RENDERER BEGINS
    if (m_tileSaver.bGoTiling)
    {
        ofPushMatrix();
        ofScale(1, -1);
        m_inkRenderer.draw(true);
        ofPopMatrix();
    }
    else
    {
        m_inkRenderer.draw(false);
    }
    ofSetColor(255, 70);
    if (m_displayLines)
    {
    ...
    m_tileSaver.end(); //-------------------------------------------------------------- TILE RENDERER ENDS
...
void ofApp::keyPressed(int key)
{
    switch (key) {
        case 's':
            m_tileSaver.finish("frame_" + ofToString(ofGetFrameNum()) + "_high.png", false);
            break;
        case 'd':
